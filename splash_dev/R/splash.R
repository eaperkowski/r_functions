  ...
  while (diff_sm > 1.0) {
    for (i in seq(from = 1, to = mdat$num_lines, by = 1)) {
      ...
      my_vals <- quick_run(mdat$lat_deg, mdat$elv_m, i, mdat$year, wn,
                           mdat$sf[i], mdat$tair[i], mdat$pn[i],kWm = mdat$kWm)
      dtot$wn[i] <- my_vals$sm
    }
    ...
    end_vals <- quick_run(mdat$lat_deg, mdat$elv_m, 1, mdat$year,
                          ...
                          mdat$tair[1], mdat$pn[1], kWm = mdat$kWm)
    diff_sm <- abs(end_vals$sm - start_sm)
...
quick_run <- function(lat, elv, n, y, wn, sf, tc, pn, kCw = 1.05, kWm) {
...
run_one_day <- function(lat, elv, n, y, wn, sf, tc, pn, kCw = 1.05, kWm) {
  # Return values
  rvals <- list()
